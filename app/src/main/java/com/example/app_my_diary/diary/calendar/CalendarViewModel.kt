...
import com.example.app_my_diary.diary.calendar.model.Month
...
class CalendarViewModel(private val app: Application) : ViewModel() {
                    ...
                    async {
                        it.days.forEach { day ->
                            if (day.date <= mCurrentDay) {
                                async {
                                    day.hasDiary = repository.checkDateHasDiary(
                                        convertToMillis(
    ...
    fun initMonthList(
        year: Int, month: Int, type: Int = NOW, selectedDate: GregorianCalendar? = null
    ): MutableList<Month> {
    ...
    fun initDateList(
        year: Int, month: Int, selectedDate: GregorianCalendar? = null, type: Int = NOW
    ): MutableList<Date> {
        ...
        val dayOfWeek: Int = getWeekDay(year, month)

        var selfDaysEndWeek = 0
        mStartBelong = true
        if (dayOfWeek != 0) {
            val startDate = endDate - dayOfWeek + 1
            var i = startDate
            var j = 0
            while (i <= endDate) {
        ...
        run {
            ...
            while (i <= days) {
                ...
                if (selectedDate != null) {
                    if (i == selectedDate.get(Calendar.DAY_OF_MONTH) && month == selectedDate.get(
                            Calendar.MONTH
                        ) + 1 && year == selectedDate.get(Calendar.YEAR)
                    ) {
