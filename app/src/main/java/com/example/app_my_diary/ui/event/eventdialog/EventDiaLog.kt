...
import com.example.app_my_diary.base.BaseViewModelBottomSheetDiaLog
import com.example.app_my_diary.databinding.LayoutEventDialogBinding
...
import com.example.app_my_diary.utils.setDayOfWeekstEventTime
import com.example.app_my_diary.utils.setEventTime
import com.example.app_my_diary.utils.setMonthEventTime
...
class EventDiaLog : BaseViewModelBottomSheetDiaLog<LayoutEventDialogBinding>(),
    ...
    fun isAnnually() {
        calendar = GregorianCalendar(
            binding!!.yearPicker.value, binding!!.monthPicker.value, binding!!.dayPicker.value
        )
    }
    ...
    fun isDayCount(): Boolean {
        ...
        return (calendar!!.get(Calendar.YEAR) < todayCalendar.get(Calendar.YEAR)) || ((calendar!!.get(
            Calendar.YEAR
        ) == todayCalendar.get(Calendar.YEAR)) && (calendar!!.get(Calendar.DAY_OF_YEAR) <= todayCalendar.get(
    ...
    private fun checkDayLeft() {
        calType = 1
        binding!!.btnDayCountShow.text = "Ngày còn lại"
    ...
    fun checkDayCount() {
        calType = 2
        binding!!.btnDayCountShow.text = "Đếm ngày"
    ...
    fun checkDayAnnually() {
        calType = 3
        binding!!.btnDayCountShow.text = "Hàng năm"
        binding!!.selectedDayLeft.visibility = View.GONE
        binding!!.selectedDayCount.visibility = View.GONE
            ...
            if (calType == 1) {
                if (isDateLeft()) {
                    ...
                    ToastUtils.getInstance(requireActivity())
                        .showToast("Không thể chọn ngày trước ngày hiện tại")
                }
            }
            if (calType == 2) {
                if (isDayCount()) {
                    ...
                    ToastUtils.getInstance(requireActivity())
                        .showToast("Không thể chọn ngày sau ngày hiện tại")
                }
            }
            if (calType == 3) {
                isAnnually()
    ...
    private fun addEvent() {
        Log.d("TAG", "addEvent: 111111")
        var title = eventModel!!.title
        var date = eventModel!!.date
        if (binding!!.etNameEvent.text!!.trim().isEmpty()) {
            Log.d("TAG", "addEvent: 2222")
            binding!!.nameEmpty = true
            focusEdittext(binding!!.etNameEvent)
            return
        } else {
            Log.d("TAG", "addEvent: 3333")
            title = binding!!.etNameEvent.text.toString().trim()
            if (calType == 1) {
                Log.d("TAG", "addEvent: 4444")
                if (isDateLeft()) {
                    binding!!.date.setEventTime(calendar!!.time.time)
                    date = calendar!!.time.time
                } else {
